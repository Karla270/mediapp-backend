...
public class ExamController {
	...
	public ResponseEntity<List<ExamDTO>> findAll() {
		List<ExamDTO> list = service.findAll().stream().map(p -> mapper.map(p, ExamDTO.class))
	...
	public ResponseEntity<Void> save(@Valid @RequestBody ExamDTO dto) {
		...
		URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(p.getIdExam())
				.toUri();
	...
	public EntityModel<ExamDTO> findIdHateos(@PathVariable("id") Integer id){
		ExamDTO dtoResponse;
		Exam obj = service.findById(id);
		if (obj == null) {
			throw new ModelNotFoundException("ID NOT FOUND: " + id);
